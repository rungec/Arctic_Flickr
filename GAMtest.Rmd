...
```{r by year, cache=T, dependson="packages and data"}
...
plotData_equaln <- PUD_grid(equaln_sample_flickr, equaln_sample_flickr$yearseason, grid_footprint) %>% 
  group_by(yearseason) %>% 
  summarise(footprint=mean(PUD>0) )%>% 
...
load(paste0("gridYearPUD_models",10000,"_m.Rdata"))
