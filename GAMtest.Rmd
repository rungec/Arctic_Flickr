...
countries <- ne_countries(scale="large",returnclass = "sf")  %>% 
  filter(adm0_a3 %in% c("CAN","USA","ISL","NOR","FRO","FIN","GRL","SJM","SWE","RUS")) %>% 
  st_transform(st_crs(flickramap)) %>% 
  st_intersection(bb) 
...
```{r hex grid}
cell_diameter <- 10000 #TODO: when doing for real, make this smaller
...
```{r overlaps,message=FALSE}
# protected areas
grid$propPA <- prop_overlap(grid,PAbuf)
grid$PA <- grid$propPA>0.2

# country
grid$country <- most_overlap(grid,countries,"sovereignt","Ocean")
...
```{r by year}
#  grid by year
gridYear <- sapply(split(flickramap, flickramap$year),
...
gridYear_equaln <- sapply(split(equaln_sample_flickr, equaln_sample_flickr$year),
                          function(x) lengths(st_intersects(grid,x))) %>% 
  cbind(grid,.) %>% 
  gather(key=year,value=photoCount,starts_with("X")) %>% 
  mutate(year=as.numeric(substr(year,2,5)))
...
table(trafficn_sample_flickr$year)
table(flickramap$year)

gridYear_trafficn <- sapply(split(trafficn_sample_flickr, trafficn_sample_flickr$year),
                          function(x) lengths(st_intersects(grid,x))) %>% 
  cbind(grid,.) %>% 
  gather(key=year,value=photoCount,starts_with("X")) %>% 
  mutate(year=as.numeric(substr(year,2,5)))
...
Let's see if we can see what is driving these changes

```{r GAM}
save(gridYear,file = paste0("gridYear_",cell_diameter,"_m"))
