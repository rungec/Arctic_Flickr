...
g1 <- PUD_grid(flickramap, flickramap$yearseason, grid_models)
#The problem: This works, but it drops the time variable in the output 
#the output consists of a long df row.id, PUD, Latitude, Longitude, which is what we want I think

#This doesn't work, I don't know why
 PUD_grid <- function(flickrrecords, timevar, grid){
   flickrrecords %>% nest(!! -quos(timevar)) %>% 
     map( function(x){
            x$col.id <- as.integer(row.names(x))
            grid$row.id <- as.integer(row.names(grid))
...
g2 <- PUD_grid(flickramap, flickramap$yearseason, grid_models)
