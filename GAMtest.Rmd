...
```{r by year, cache=T, dependson="packages and data", warning=F}
...
save(byYear_models,file = paste0("flickr/byYear_models",10000,"_m.Rdata"))
...
save(byYear_footprint,file = paste0("flickr/byYear_footprint",5000,"_m.Rdata"))
...
```{r footprint2, cache=T, warning=F}
# or
...
```{r GAM, cache=T, dependson="by year", eval=FALSE}

load("flickr/gridYearPUD_models10000_m.Rdata")

gridYearmod <- gridYearPUD_models %>% 
  filter(country!="Russia") %>% 
  mutate(PUDlog10=log10(PUD+1),
         country=as.factor(country))

#set norway as the reference level
gridYearmod <- within(gridYearmod, country <- relevel(country, ref = "NOR"))
...
```{r GAM_seasonal, cache=T, dependson="by year", eval=FALSE}

gridYearsummermod <- gridYearmod %>% 
  filter(season=="summer") 

gridYearwintermod <- gridYearmod %>% 
  filter(season=="winter") 
