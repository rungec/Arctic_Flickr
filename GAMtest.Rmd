...
PUD_grid <- function(flickrrecords, time, grid){
  ...
  df <- df %>%       
        ...
        mutate(year=as.numeric(strsplit(yearseason, "_")[[1]][1]),
...
adj_fun <- function(gridYearPUD){
          byYear <- gridYearPUD %>%
                      data.frame() %>% 
                      group_by(yearseason) %>% 
                      summarise("Artic PUD"=sum(PUD)) %>% 
                      left_join(AllFlickr,by="yearseason") %>% 
                      mutate("Percent Traffic"=.$"Artic PUD"/.$"Total Flickr Photos",
...
gridYearPUD_models <- adj_fun(gridYearPUD_models)
save(gridYearPUD_models,file = paste0("gridYearPUD_models",cell_diameter,"_m.Rdata"))
...
gridYearPUD_footprint <- adj_fun(gridYearPUD_footprint)
save(gridYearPUD_footprint,file = paste0("gridYearPUD_footprint",cell_diameter,"_m.Rdata"))
...
#reorder
plotData$variable <- ordered(plotData$variable,levels=c("Total Flickr Photos","Artic PUD","Percent Traffic"))
ggplot(plotData) + 
