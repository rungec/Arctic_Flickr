...
# though we use amap boundaries in the analysis, I rasterise the data based on the maximum extent of the data
# so that if we revise the study region I don't have to redo the rasterization.
...
rastfun <- function(data, es, curres, currfile) {
  ...
  rasttemplate <- raster(xmn=-3335000, xmx=3335000, ymn=-3335000, ymx=3335000, res=curres, crs=rcrs)
    if(file.exists(sprintf("%s/Boundaries/Arctic_circle/60degreesN/60degreesN_%smres.tif", wd2, curres))==FALSE){ 
      ...
      rast60N[rast60N==1] <- 0
    #fill with pud
    esrast <- rasterize(data_es, rasttemplate, fun=function(x, ...){ length(unique(x))}, field="owner_date", update=TRUE, filename=sprintf("Flickr_%s_per%scell.tif", currfile, currres), overwrite=TRUE)
  }
...
biorast <- rastfun(flickramap, bioes, 1000, "biotic_all")
