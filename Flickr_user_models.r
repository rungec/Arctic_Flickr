...
g5 <- lmer(sqrtdist2road ~ sqrtwildphotoprop + nearPA + wildlifebird_photos +
             (1|owner), data = accessdf, REML=FALSE)
g5r <- lmer(sqrtdist2road ~ sqrtwildphotoprop + nearPA + wildlifebird_photos +
              (1|owner), data = accessdf, REML=TRUE)

sink("Model_of_wildlifephotoprop_byaccess_lme.txt", append=TRUE)
print("Sqrt transformed model, estimated by REML")
print(summary(g5r))
print(car::Anova(g5r)) #p values for the fixed effects
sink()

#sqrt dist2road model
predsdf <- get_model_data(g5r, type="pred", terms=c("sqrtwildphotoprop [0, 0.1, 0.1581139, 0.2236068, 0.3162278, 0.3872983, 0.4472136, 0.5, 0.5477226, 0.6324555, 0.7071068, 0.7745967, 0.83666, 0.8944272, 0.9486833, 1]", "wildlifebird_photos", "nearPA")) %>%
  mutate(pred_transform = (predicted^2)*1000,
         confhigh_transform = (conf.high^2)*1000,
         conflow_transform = (conf.low^2)*1000,
         wildphotoprop=x^2)
names(predsdf)[6:7] <- c("wildlifebird_photos", "nearPA")
levels(predsdf$nearPA) <- c("Outside PA", "Near or inside PA")
levels(predsdf$wildlifebird_photos) <- c("Non-wildlife photos", "Wildlife or bird photos")
write.csv(predsdf, "Model_of_wildlifephotoprop_byaccess_lme_sqrtmod_predictions.csv", row.names=FALSE)
...
ggplot(predsdf, aes(x=wildphotoprop, y=pred_transform, group=wildlifebird_photos)) +
  ...
  theme_minimal(16) +
  theme(legend.title=element_blank(), legend.position = "bottom")
ggsave("Model_of_wildlifephotoprop_byaccess_lme_sqrtmodel_transformedpredictions.png", width=7.77, height=4.34, units="in")

# plot marginal effects 
p51 <- plot_model(g5r, type = "pred", terms = "sqrtwildphotoprop")
p52 <- plot_model(g5r, type = "pred", terms = "wildlifebird_photos")
p53 <- plot_model(g5r, type = "pred", terms = "nearPA")
pout <- ggarrange(p51, ggarrange(p52, p53, ncol = 2, labels = c("B", "C")), nrow=2, labels="A")
ggsave("Model_of_wildlifephotoprop_byaccess_lme_marginaleffects_sqrtmod.png", pout, width=7, height=9, units="in", dpi=300)

p5<-plot_model(g5r, type = "diag")
pout <- ggarrange(p[[1]], p[[3]], p[[4]], ncol=2, nrow=2, labels = c("A", "B", "C"))
ggsave("Model_of_wildlifephotoprop_byaccess_lme_diag_sqrtmod.png", pout, width=9, height=7, units="in", dpi=300)
